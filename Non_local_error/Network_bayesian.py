...
class Place_net(bp.DynamicalSystem):
    def __init__(self, z_min, z_max, get_center_max=False, num=1000, k = 1.,
                 tau=10.,  a_p = 4., J0=50.):
        ...
        self.get_center_max = get_center_max
    ...
    def get_center(self):
        ...
        if self.get_center_max:
            max_index = bm.argmax(r)
            self.center[0] = self.x[max_index]
        else:
...
class Place_net_crm(bp.DynamicalSystem):
    def __init__(self, z_min, z_max, num=1000):
        super(Place_net_crm, self).__init__()

        # parameters
        self.num = num
        # feature space
        self.x = bm.linspace(z_min, z_max, num, endpoint = False)  
        # variables
        self.r = bm.Variable(bm.zeros(num))
        self.center = bm.Variable(bm.zeros(1))
        # Connections
    ...
    def update(self, I_grid):
        # Calculate self position
        max_index = bm.argmax(I_grid)
        self.center[0] = self.x[max_index]
        r = bm.zeros(self.num)
        r[max_index] = 1
        self.r.value = r
...
class Coupled_Net_crm(bp.DynamicalSystemNS):
    def __init__(self, HPC_model, MEC_model_list, num_module):
        super(Coupled_Net_crm, self).__init__()
        ...
        self.I_mec = bm.Variable(bm.zeros([self.num_hpc, ]))
    ...
    def update(self, alpha_g, Ig):
        ...
        self.HPC_model.update(I_grid = self.I_mec)
...
class Coupled_Net(bp.DynamicalSystemNS):
    ...
    def update(self, alpha_p, alpha_g, Ip, Ig):
        ...
        index = bm.argmax(r_hpc)
        kernel = bm.zeros(self.num_hpc)
        for j in range(10):
            kernel[j+index-5] = 1
        self.I_mec.value = I_mec*kernel
        # Update Hippocampus states

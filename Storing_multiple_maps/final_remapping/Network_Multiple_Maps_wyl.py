...
class Place_net(bp.DynamicalSystem):
    def __init__(
        ...
        for i in range(self.map_num):
            ...
            conn_mat = bm.fill_diagonal(conn_mat, 0, inplace=False)
            if i >= 1:
    ...
    def get_bump(self, map_index, center):
        d = self.period_bound(center - self.map[map_index, :]).reshape(
            -1,
        )
    ...
    def get_center(self, map_index):
        ...
        exppos = bm.exp(1j * x)
        self.center[0] = (
            bm.angle(bm.sum(exppos * r)) / 2 / bm.pi + 1 / 2
        ) * self.z_range + self.z_min

    def update(self, loc=0.0, map_index=0, input_stre=1, input_g=0.0):
    ...
    def update_phi(self, map_index=0, input_g=0.0):
        # Calculate self position
        self.get_center(map_index=map_index)
        # Calculate recurrent input
        Irec = bm.matmul(self.conn_mat, self.r)
        # Update neural state
        noise = self.noise_stre * bm.random.randn(self.neuron_num)
        du = (-self.u + Irec + noise + input_g) / self.tau * bm.dt
        u = self.u + du
        self.u.value = bm.where(u > 0, u, 0)
        r1 = bm.square(self.u)
        r2 = 1.0 + self.k * bm.sum(r1)
...
class Grid_net(bp.DynamicalSystem):
    def __init__(
        ...
        self.x = bm.linspace(
            -bm.pi, bm.pi, neuron_num, endpoint=False
        ...
        for map_index in range(self.map_num):
            ...
            if map_index >= 1:
                out -= bm.mean(out)
            conn_out[
                self.place_index[map_index], :
            ] += out  # From grid cells to place cells
    ...
    def make_conn_out(self, map_index):
        ...
        conn_out = (
            self.W0
            / (self.a * bm.sqrt(2 * bm.pi))
            * bm.exp(-0.5 * bm.square(Dis_circ / self.a))
        )
    ...
    def update(self, r_hpc, loc=10, input_stre=1.0, map_index=0, I_per=0):
        ...
        noise = self.noise_stre * bm.random.randn(self.neuron_num)
        # Calculate total input
        self.input.value = input_hpc + Ig + noise
        # Update neural state
    ...
    def update_phi(self, r_hpc, phase_input, input_stre=1.0):
        Ig = self.get_input_phase(phase_input=phase_input, input_strength=input_stre)
        # Calculate self position
        self.get_center()
        # Calculate hpc input
        conn_in = bm.transpose(self.conn_out)
...
class Coupled_Net(bp.DynamicalSystemNS):
    def __init__(self, HPC_model, MEC_model_list, neuron_num_module):
        ...
        self.W_G = bm.ones(
            [
                self.neuron_num_module,
            ]
        )
        self.phase = bm.Variable(
            bm.zeros(
                [
                    self.neuron_num_module,
                ]

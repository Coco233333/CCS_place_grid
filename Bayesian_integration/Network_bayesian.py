...
class Place_net(bp.DynamicalSystem):
    def __init__(self, z_min, z_max, num=1000, k=1.0, tau=10.0, a_p=4.0, J0=50.0):
        ...
        self.x = bm.linspace(
            z_min, z_max, num, endpoint=False
    ...
    def get_center(self):
        ...
        exppos = bm.exp(1j * x)
        self.center[0] = (
            bm.angle(bm.sum(exppos * r)) / 2 / np.pi + 1 / 2
...
class Grid_net(bp.DynamicalSystem):
    def __init__(
        ...
        # feature space
        self.x_hpc = np.linspace(
            z_min, z_max, num_hpc, endpoint=False
        )  # The encoded feature values
        self.x = np.linspace(
            -np.pi, np.pi, num, endpoint=False
        )  # The encoded feature values
...
class Coupled_Net(bp.DynamicalSystemNS):
    ...
    def Get_Energy(self, alpha_p, alpha_g, Ip, Ig):
        ...
        for i in range(self.num_module):
            ...
            sigma_phi[i] = 1 / (
                (Lambda[i] / 2 / bm.pi) * bm.sqrt(J_pg * rho_g * Rg / (4 * Ap * tau_p))
            )
        ...
        # Circumference distance function
        ...
        for i in range(self.num_module):
            ...
            log_prior -= (
                1 / (sigma_phi[i] ** 2) * bm.exp(-(dis_2**2) / 8 / a_g**2) * dis_2**2
            )
        dis_x = place_x - z

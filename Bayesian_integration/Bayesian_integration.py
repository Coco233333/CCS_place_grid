...
# Simulation of different noise levels
...
np.savez('./data/results.npz', 
         error_int_net=error_int_net,
         error_mot_net=error_mot_net,
         error_vis_net=error_vis_net,
         error_int_gop=error_int_gop,
...
plt.tight_layout()
plt.savefig('./data/decoding_error.png')
plt.show()


# simulation under noise level 0.2
...
for i in range(trial_num):
    ...
    z_decode_gop[i] = z_decode_g[-1]
    z_decode_net[i] = z_decode_n[-1].item()
    if i % 5 == 0:

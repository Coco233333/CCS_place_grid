...
def Grid_tuning_generation(phi_candidate, a_g, num_g):
    ...
    theta = np.linspace(
        0, 2 * np.pi, num_g, endpoint=False
    )  # Assuming theta is linspace
...
def loglikelihood_Ig(Ig, fg, sigma_g):
    n_phi = fg.shape[-1]
    # Use expand_dims to expand the matrix to (n, m, 1)
    ...
    log_prob = (
        -0.5 * (Ig_expand - fg) ** 2 / sigma_g[:, None, None] ** 2
    )  # shape [M, n_g, n_phi]
...
def Simplified_PSC_MAP_decoder(
    activation_gs,
    n_pos=10000,
    n_phi=500,
    M=3,
    ...
    fg_modules = Grid_tuning_generation(
        phi_candidates_modules, a_g, num_g
    )  # shape [M, n_g, n_phi]

    log_likelihood_fr = loglikelihood_Ig(
        activation_gs, fg_modules, sigma_g=sigma_g_infer
    )  # shape [M, n_phi]
